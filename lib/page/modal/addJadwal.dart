...
class ModalJadwalMengajar extends StatefulWidget {
  ...
  const ModalJadwalMengajar({Key key, this.idKelas, this.idJadwal, this.onSubmit}) : super(key: key);
...
class _ModalJadwalMengajarState extends State<ModalJadwalMengajar> {
  ...
  void addJadwal() async {
    ...
    bool status = await Provider.of<KelasProvider>(context, listen: false).addJadwal(widget.idKelas, data);
    if (status) {
      txtMulai.text = "";
      txtSelesai.text = "";
      // _showSuccesAdd();
      setState(() {
        Config.alert(1, 'Berhasil menambah jadwal');
                ...
                children: [
                  Expanded(
                    ...
                    child: TextButton(
                        onPressed: () {
                          if (valHari == null) {
                            Config.alert(0, 'Pilih hari terlebih dahulu');
                          } else if (txtMulai.text.isEmpty) {
                            Config.alert(0, 'Pilih jam mulai terlebih dahulu');
                          } else if (txtSelesai.text.isEmpty) {
                            Config.alert(0, 'Pilih jam selesai terlebih dahulu');
                          } else {
                            addJadwal();
                          }
                        },
